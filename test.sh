...
cases=(tree events bridge mount shelf menu system roles text pixels applied leaks enabled checked controls numbers strings table permission opacity clock frames anim gpu triangle canvas shader)
...
cross_host=(roles events text applied leaks enabled checked controls numbers strings table permission opacity clock anim gpu canvas shader)
...
if [[ $native -eq 1 && $have_host -eq 1 ]]; then
    ...
    # copied it.
    for example in hello clock shader canvas signin brew ledger finder about settings permissions; do
