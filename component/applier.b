...
///
/// The only class in `cortado.component` that touches a control. Everything
...
pub class Applier {
    ...
    pub fn init(root: widgets.Widget, router: events.EventRouter, owner: Mount) {
        self.root = root
        self.router = router
        self.owner = owner
    }
    ...
    fn one(change: Change) -> Result<bool> {
        ...
        match change.kind {
            ...
            bind => {
                ...
                let kind: events.EventKind = change.event
                self.router.on(target.handle(), kind,
                    fn(event: events.UiEvent) { owner.dispatch(handle, kind, event) })
                return ok(true)
            }
            unbind => {
    ...
    fn subscribe(element: Element, control: widgets.Widget) -> Result<bool> {
        var index: int = 0
        for index: int in 0..element.listener_count() {
            let owner: Mount = self.owner
            let handle: u64 = control.handle().raw
            let kind: events.EventKind = element.listener_at(index).kind
    ...
    fn resolve(change: Change) -> Result<widgets.Widget> {
        if change.at_surface {
            return ok(self.root)
        }
        var here: widgets.Widget = self.face()?
        var step: int = 0
