...
/// it, so cortado's hosts do not write a pane's frame at all. What this layout
/// does is compute the *same* two boxes, so that everything inside a pane —
/// which is ordinary cortado layout — is solved against the size the pane is
...
/// plain numbers on this class because `cortado.layout` makes no host calls —
...
pub class SplitLayout extends Layout {
    ...
    pub override fn arrange(node: LayoutNode, content: geometry.Rect,
        ...
        // divider is an ordinary thing for a user to do. What cortado refuses
