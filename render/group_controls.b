...
pub class DisclosureRender extends BoxRender {
    ...
    pub override fn set_integer(key: int, value: int) -> Result<bool> {
        if key != host.P_EXPANDED { return super.set_integer(key, value) }
        self.demand_alive()?
    ...
    pub override fn integer(key: int) -> Result<int> {
        if key == host.P_EXPANDED { self.demand_alive()?; return ok(if self.expanded { 1 } else { 0 }) }
        return super.integer(key)
    }
    pub override fn set_value_as_user(index: int, value: f64) -> Result<bool> {
        return self.set_integer(host.P_EXPANDED, index)
    }
    pub override fn measure(available: geometry.Size) -> Result<geometry.Size> {
        let paragraph: paint.Paragraph = self.renderer.paragraph(self.words, self.font_size(), -1.0, self.text_color())?
    ...
    pub override fn handle_event(event: events.UiEvent) -> Option<events.UiEvent> {
        ...
        if event.kind == events.EventKind.pointer_down && event.index == host.BTN_LEFT && event.position.y < 30.0 {
        ...
        if event.kind == events.EventKind.key_down && (event.key() == events.Key.space || event.key() == events.Key.ret) { return self.toggle() }
        return none
    }
}
