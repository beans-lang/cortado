...
pub class UiContext {
    ...
    pub fn init(renderer: paint.Renderer, namespace: int) {
        ...
        self.router_value = new events.EventRouter(false)
    ...
    pub fn router() -> events.EventRouter { return self.router_value }
    ...
    fn forget_tree(object: RenderObject) {
        ...
        self.router_value.forget(host.Handle.of(object.handle()))
    }
    ...
    pub fn key(root: RenderObject, kind: events.EventKind, key: events.Key, text: string, modifiers: int) -> Result<bool> {
        match self.popup_value.root() {
            some(_) => {
                if kind == events.EventKind.key_down && (key == events.Key.escape || key == events.Key.tab) {
                    self.popup_value.dismiss()
                    if key == events.Key.escape { return ok(true) }
                }
            }
            none => {}
        }
