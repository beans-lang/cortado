...
pub abstract class ToggleRender extends RenderObject {
    ...
    pub override fn measure(available: geometry.Size) -> Result<geometry.Size> {
        ...
        return ok(geometry.Size.of(paragraph.size().width + 38.0, if paragraph.size().height > 24.0 { paragraph.size().height } else { 24.0 }))
    }
...
pub class LevelIndicatorRender extends RangeRender {
    ...
    pub override fn measure(available: geometry.Size) -> Result<geometry.Size> { return ok(geometry.Size.of(140.0, 16.0)) }
}
