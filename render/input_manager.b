...
pub class InputManager {
    ...
    router_value: events.EventRouter
    focus_value: FocusManager
    invalidation_value: Invalidation
    hover_value: HoverManager
    captured_handle: u64 = 0
    pub fn init(registry: RenderRegistry, router: events.EventRouter, focus: FocusManager, dirty: Invalidation) {
    ...
    pub fn set_value_as_user(handle: u64, index: int, value: f64) -> Result<bool> {
        match self.registry_value.get(handle) {
            ...
            some(object) => {
                ...
                let event: events.UiEvent = events.UiEvent.of(events.EventKind.value_changed, host.Handle.of(handle))
    ...
    pub fn commit_cell(handle: u64, row: int, column: int, text: string) -> Result<bool> {
        match self.registry_value.get(handle) {
            ...
            some(object) => {
                ...
                let event: events.UiEvent = events.UiEvent.of(events.EventKind.text_commit, host.Handle.of(handle))
    ...
    pub fn key(root: RenderObject, kind: events.EventKind, key: events.Key, text: string, modifiers: int) -> Result<bool> {
        ...
        if kind == events.EventKind.key_down && key == events.Key.tab {
            return self.focus_value.advance(root, (modifiers & host.MOD_SHIFT) != 0)
        }
        if self.focus_value.current() == 0 { return ok(false) }
        let event: events.UiEvent = events.UiEvent.of(kind, host.Handle.of(self.focus_value.current()))
        event.index = key.code(); event.text = text; event.modifiers = modifiers
        return self.dispatch(event)
    }
    pub fn dispatch(event: events.UiEvent) -> Result<bool> {
        match self.registry_value.get(event.target.raw) {
            none => { return err("event targets a stale render object", "stale") }
            some(object) => {
                if !self.focus_value.interactive(object) { return ok(false) }
                let produced: Option<events.UiEvent> = object.handle_event(event)
    ...
    pub fn pointer(root: RenderObject, kind: events.EventKind, position: geometry.Point, button: int, clicks: int = 1, modifiers: int = 0) -> Result<bool> {
        ...
        match target {
            ...
            some(object) => {
                if kind == events.EventKind.pointer_down {
