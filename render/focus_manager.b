...
pub class FocusManager {
    ...
    pub fn init(registry: RenderRegistry, router: events.EventRouter) {
            ...
            some(object) => {
                ...
                match self.registry_value.get(self.focused_handle) {
                    some(previous) => {
                        ...
                        self.router_value.deliver(events.UiEvent.of(events.EventKind.blur, host.Handle.of(previous.handle())))
                    }
                    none => {}
                }
                self.focused_handle = handle
                object.focus_changed(true)
                self.router_value.deliver(events.UiEvent.of(events.EventKind.focus, host.Handle.of(handle)))
                return ok(true)
            }
        }
    }
    ...
    pub fn clear() {
        ...
        match self.registry_value.get(previous) {
            some(object) => {
                ...
                self.router_value.deliver(events.UiEvent.of(events.EventKind.blur, host.Handle.of(previous)))
            }
            none => {}
        }
    }
