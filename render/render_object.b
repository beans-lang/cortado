...
pub abstract class RenderObject {
    ...
    pub fn string_at(key: int) -> Result<string> {
        self.demand_alive()?
        if key == host.S_A11Y_LABEL { return ok(if self.a11y_name == "" { self.words } else { self.a11y_name }) }
        return err("string property is not supported by this shared control", "unsupported")
    }
    pub fn set_integer(key: int, value: int) -> Result<bool> {
        self.demand_alive()?
        if key == host.P_ENABLED { self.enabled = value != 0 }
        else if key == host.P_HIDDEN { self.hidden = value != 0 }
        else if key == host.P_FOCUSABLE { self.focusable = value != 0 }
    ...
    pub fn integer(key: int) -> Result<int> {
        self.demand_alive()?
        if key == host.P_ENABLED { return ok(if self.enabled { 1 } else { 0 }) }
        if key == host.P_HIDDEN { return ok(if self.hidden { 1 } else { 0 }) }
        if key == host.P_FOCUSABLE { return ok(if self.focusable { 1 } else { 0 }) }
    ...
    pub fn handle_event(event: events.UiEvent) -> Option<events.UiEvent> { return none }
