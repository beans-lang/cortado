...
pub class TabViewRender extends BoxRender {
    ...
    pub override fn integer(key: int) -> Result<int> {
        self.demand_alive()?
        if key == host.P_BORDERLESS { return ok(if self.borderless_value { 1 } else { 0 }) }
        if key == host.P_SELECTED { return ok(self.chosen) }
    ...
    pub override fn set_value_as_user(index: int, value: f64) -> Result<bool> {
        ...
        return self.set_integer(host.P_SELECTED, index)
    }
    ...
    pub override fn handle_event(event: events.UiEvent) -> Option<events.UiEvent> {
        ...
        if event.kind == events.EventKind.pointer_down && event.index == host.BTN_LEFT && event.position.y < 30.0 { self.tracking = true; return none }
        ...
        if event.kind == events.EventKind.key_down && self.child_count() > 0 {
            if event.key() == events.Key.right { return self.select_as_user((self.chosen + 1) % self.child_count()) }
            if event.key() == events.Key.left { return self.select_as_user((self.chosen + self.child_count() - 1) % self.child_count()) }
        }
        return none
    }
...
pub class SplitViewRender extends BoxRender {
    ...
    pub override fn set_integer(key: int, value: int) -> Result<bool> {
        if key != host.P_AXIS { return super.set_integer(key, value) }
        self.demand_alive()?
    ...
    pub override fn integer(key: int) -> Result<int> {
        if key == host.P_AXIS { self.demand_alive()?; return ok(if self.stacked_value { 1 } else { 0 }) }
        return super.integer(key)
    }
    pub override fn set_real(key: int, value: f64) -> Result<bool> {
        if key != host.P_DIVIDER { return super.set_real(key, value) }
        self.demand_alive()?
    ...
    pub override fn real(key: int) -> Result<f64> {
        if key == host.P_DIVIDER { self.demand_alive()?; return ok(self.divider_value) }
        return super.real(key)
    }
    pub override fn set_value_as_user(index: int, value: f64) -> Result<bool> { return self.set_real(host.P_DIVIDER, value) }
    ...
    pub override fn handle_event(event: events.UiEvent) -> Option<events.UiEvent> {
        ...
        if event.kind == events.EventKind.pointer_up { self.tracking = false }
        if event.kind == events.EventKind.pointer_move && self.tracking {
