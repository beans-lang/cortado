Cortado's shared control visuals are compiled from these `.bx` files.

Run `build/cortado-bx build templates` after editing them. Generated Beans is
committed under `generated/templates/` so an application does not need to run
the markup compiler just to import Cortado. `tools/test_rendered.py` checks
