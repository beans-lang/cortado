...
/// control cortado has that is not on every platform**. The Win32 common
...
/// `WidgetKind.switch.available()` is the question on its own. What cortado
...
pub class Switch extends Widget {
    ...
    /// platform.
    pub static fn of(on: bool) -> Result<Switch> {
        WidgetKind.switch.demand()?
        var control: Switch = new Switch()
        control.set_on(on)?
        return ok(control)
    }

    pub fn set_on(on: bool) -> Result<bool> {
        return self.set_flag(host.P_CHECKED, on, "flip a switch")
    }

    pub fn is_on() -> Result<bool> {
        return self.read_flag(host.P_CHECKED, "read whether a switch is on")
    }
}
