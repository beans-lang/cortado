...
pub class SearchField extends Widget {
    ...
    pub static fn of(hint: string) -> Result<SearchField> {
        ...
        var field: SearchField = new SearchField()
        field.set_hint(hint)?
        return ok(field)
    }
    ...
    pub fn set_hint(hint: string) -> Result<bool> {
        return self.set_string(host.S_HINT, hint, "set a search field's hint")
    }

    pub fn hint() -> Result<string> {
        return self.string_at(host.S_HINT, "read a search field's hint")
    }
