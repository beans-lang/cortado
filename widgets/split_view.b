...
/// which is what cortado will not do), and UIKit's split view is a view
...
/// out of it, so cortado's hosts do not write a pane's frame — they write
...
pub class SplitView extends ChildHolder {
    ...
    pub static fn of(stacked: bool) -> Result<SplitView> {
        WidgetKind.split_view.demand()?
        var split: SplitView = new SplitView()
    ...
    pub fn set_stacked(stacked: bool) -> Result<bool> {
        return self.set_property(host.P_AXIS, if stacked { 1 } else { 0 })
    }

    pub fn is_stacked() -> Result<bool> {
        return self.read_flag(host.P_AXIS, "read which way a split view divides")
    }

    /// Where the handle sits, in points from the leading edge.
    pub fn set_divider(where: f64) -> Result<bool> {
        return self.set_property_real(host.P_DIVIDER, where)
    }

    pub fn divider() -> Result<f64> {
        return self.read_real(host.P_DIVIDER, "read where a split view's handle is")
    }
    ...
    /// not have — and because `cortado.layout` cannot ask a control anything,
    /// by design.
