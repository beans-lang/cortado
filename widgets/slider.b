...
pub class Slider extends Widget {
    ...
    pub static fn of(low: f64, high: f64, start: f64) -> Result<Slider> {
        var control: Slider = new Slider()
    ...
    pub fn value() -> Result<f64> {
        return self.read_real(host.P_VALUE, "read a slider's value")
    }

    pub fn low() -> Result<f64> {
        return self.read_real(host.P_MIN, "read a slider's lowest value")
    }

    pub fn high() -> Result<f64> {
        return self.read_real(host.P_MAX, "read a slider's highest value")
    }
