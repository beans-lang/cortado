...
/// so cortado refuses rather than drawing a bar and calling it a gauge. Ask
...
pub class LevelIndicator extends Widget {
    ...
    pub static fn of(low: f64, high: f64, level: f64) -> Result<LevelIndicator> {
        ...
        var control: LevelIndicator = new LevelIndicator()
    ...
    pub fn set_level(level: f64) -> Result<bool> {
        return self.set_property_real(host.P_VALUE, level)
    }

    pub fn level() -> Result<f64> {
        return self.read_real(host.P_VALUE, "read a level indicator's level")
    }

    pub fn low() -> Result<f64> {
        return self.read_real(host.P_MIN, "read a level indicator's lowest value")
    }

    pub fn high() -> Result<f64> {
        return self.read_real(host.P_MAX, "read a level indicator's highest value")
    }
}
