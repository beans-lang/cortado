...
pub class ProgressBar extends Widget {
    ...
    pub static fn of(low: f64, high: f64) -> Result<ProgressBar> {
        var control: ProgressBar = new ProgressBar()
    ...
    pub fn value() -> Result<f64> {
        return self.read_real(host.P_VALUE, "read a progress bar's value")
    }
    ...
    pub fn set_indeterminate(on: bool) -> Result<bool> {
        return self.set_property(host.P_INDETERMINATE, if on { 1 } else { 0 })
    }

    pub fn is_indeterminate() -> Result<bool> {
        return self.read_flag(host.P_INDETERMINATE,
