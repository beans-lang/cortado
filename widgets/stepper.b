...
pub class Stepper extends Widget {
    ...
    /// to be given.
    pub static fn of(low: f64, high: f64, step: f64, start: f64) -> Result<Stepper> {
        var control: Stepper = new Stepper()
    ...
    pub fn set_range(low: f64, high: f64) -> Result<bool> {
        self.set_property_real(host.P_MIN, low)?
        return self.set_property_real(host.P_MAX, high)
    }

    pub fn set_value(value: f64) -> Result<bool> {
        return self.set_property_real(host.P_VALUE, value)
    }

    pub fn value() -> Result<f64> {
        return self.read_real(host.P_VALUE, "read a stepper's value")
    }

    pub fn low() -> Result<f64> {
        return self.read_real(host.P_MIN, "read a stepper's lowest value")
    }

    pub fn high() -> Result<f64> {
        return self.read_real(host.P_MAX, "read a stepper's highest value")
    }

    /// How far one press moves it. Must be more than zero.
    pub fn set_step(step: f64) -> Result<bool> {
        return self.set_property_real(host.P_STEP, step)
    }

    pub fn step() -> Result<f64> {
        return self.read_real(host.P_STEP, "read a stepper's increment")
    }
}
