...
pub class WidgetDump {
    ...
    static fn walk(widget: Widget, depth: int, inout out: fmt.StringBuilder) -> Result<bool> {
        ...
        // The two trees are compared here, every time the dump runs. cortado's
        ...
        if mine.len() != theirs {
            return err("{widget.kind().name()} has {mine.len()} children in cortado but {theirs} on the platform",
                       "tree_drift")
        }
        if theirs > 0 {
