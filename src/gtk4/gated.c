...
// of these exists on Linux: BlueZ speaks Bluetooth over D-Bus, GeoClue answers
// where the machine is, PipeWire and V4L2 enumerate cameras, and the portal
// API records the screen. None of them is GTK, and each is a service that may
// or may not be running on a given machine — which makes every one of them a
// dependency this package would acquire and a daemon a program would have to
