...
//
// The same file as the macOS host's in every line that matters, because it is
// the same framework underneath: Core Animation is Core Animation on both. The
// two differences are the view class, and that a UIView is always layer-backed
// so there is nothing here to ask for a layer — a phone composites every view
// on the GPU and has done since the first one.
//
...
ctd_handle ctd_anim_new(ctd_handle widget, int32_t property) {
    ...
    // tracked with no widget kind, and on this platform a UIWindow is a UIView
    // so nothing else here would have told them apart.
...
ctd_status ctd_anim_start(ctd_handle anim, int64_t token) {
    ...
    // Nothing asks for layer-backing here and nothing needs to: every UIView
    // has a layer from the moment it is made, which is the one real difference
    // between this file and the macOS one.
