...
//
// // Windows' browser engine is WebView2, which is a *redistributable*: the user
// has to have the Evergreen runtime installed, and a machine without it gets
// nothing. A control that appeared on a developer's box and was an empty grey
// rectangle on a user's would be worse than one that says it is not there.
//
...
ctd_status ctd_web_load(ctd_handle widget, const char *url, int32_t len) {
    (void)url; (void)len;
    return ctd_window(widget) ? CTD_ERR_KIND : CTD_ERR_STALE;
}
