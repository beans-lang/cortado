...
pub class Options {
    ...
    /// The path written into the generated file's header, when it should not
    /// be the path the file was read from.
    ///
    /// The header names its source so an author can find it, and a header that
    /// named whatever spelling the tool happened to be handed would make the
    /// generated bytes depend on the working directory — `screens/home.bx` run
    /// from the project and `./screens/home.bx` run from the same place with a
    /// dot on the front would produce two different files, and a drift gate
    /// would call one of them stale. The driver sets this to the source's path
    /// relative to its module root, which is the one spelling that is the same
    /// from everywhere.
...
pub fn compile_source(source: string, path: string, options: Options) -> Compiled {
    ...
    if options.source_label != "" { shown = options.source_label }

    let body: List<string> = []
    body.push("// Generated from {shown} by cortado. Do not edit.")
    body.push("//")
    ...
    body.push("//")
    body.push("//     cortado generate {shown}")
