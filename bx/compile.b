...
//
//     // Generated from pages/counter.bx by cortado-bx. Do not edit.
//     package pages
//     import {Builder, Component} from cortado.component
//     <the <beans> block, verbatim>
//     fn _cortado_component_counter_Hint(value: Hint) -> Component { return value }
...
// time.** cortado-bx cannot type-check, so it emits one free function per
// distinct component tag:
//
//     fn _cortado_component_counter_Hint(value: Hint) -> Component { return value }
//
// An unused free function costs nothing and is not an error. If `Hint` is not a
// `Component` the answer is `error: expected cortado.component.Component, got app.Hint`
...
/// The names the generated file imports.
///
/// `Builder` and `Component` are spelled by generated code; `UiEvent` is
/// spelled by the author, in every handler closure. An unused import is not an
/// error and a missing one is, so all three are written whether or not a given
/// file needs them.
///
/// `UiEvent` comes from a different package than the other two, which is why
/// the import line is built in two halves rather than one.
pub fn cortado_imports() -> List<string> {
    return ["Builder", "Component"]
}

/// The names the generated file imports from `cortado.events`.
pub fn event_imports() -> List<string> {
    return ["UiEvent"]
}
...
/// `site/checkout.bx` becomes `generated/site/checkout.b`.
///
/// Mirroring rather than flattening, and that is the whole design:
///
/// - **A directory is a package in Beans**, and its name is the folder's name,
///   not what a file declares. A mirrored tree therefore keeps every package
///   name it had — `site` stays `site` — and gains one prefix,
///   `generated`, at the root. Nothing has to be renamed as markup is added,
///   moved or nested.
/// - Generated code lives in one place, so `.gitignore`, a file watcher and a
///   search can each name it once.
/// - The markup folders hold only `.bx` and are not Beans packages at all, so
///   nobody edits the wrong half of a file: the two halves are not neighbours.
///
/// Paths are taken as given, so run the compiler from the module root and the
/// mirror lands where it belongs.
pub fn generated_path_for(path: string) -> string {
    return generated_path_under("generated", path)
}

/// The same, with the mirror rooted at `root` instead of `generated`.
pub fn generated_path_under(root: string, path: string) -> string {
    let stem: string = stem_of(path)
    let folder: string = directory_of(path)
    if folder == "" { return "{root}/{stem}.b" }
    return "{root}/{folder}/{stem}.b"
}

/// Where a source file's generated half goes, given the module it belongs to.
///
/// `module_root` is the directory holding `beans.pot`; `source` is the path as
/// the caller wrote it. The mirror hangs off the module root, so
/// `cortado-bx build site/checkout.bx` and
/// `cortado-bx build examples/markup/site/checkout.bx` write the same file —
/// the output does not depend on where the compiler was run from.
pub fn mirror_path(module_root: string, out_root: string,
                   source: string, relative: string) -> string {
    let under: string = generated_path_under(out_root, relative)
    if module_root == "" { return under }
    return "{module_root}/{under}"
}

/// The directory part of a path, without its trailing separator, or `""` for a
/// bare filename.
...
/// The last segment of a directory path — the package name a generated file
/// will have.
pub fn folder_name_of(folder: string) -> string {
    var cut: int = -1
    var i: int = 0
    for i < folder.len() {
        let b: int = folder.byte_at(i) as int
        if b == 47 || b == 92 { cut = i }
        i = i + 1
    }
    if cut < 0 { return folder }
    return folder.slice(cut + 1, folder.len())
}
...
pub fn compile_source(source: string, path: string, options: Options) -> Compiled {
    ...
    if !declared {
        ...
        out.diags.push(Diag.of(locate.span(),
            "{file} has to declare `partial class {wanted}` — the class is named after the file, and cortado-bx writes the other half of it. The block declares {found}"))
    }

    // An import of cortado.component that binds a name the generated line
    // already binds is a duplicate in a file the author never wrote.
    var index: int = 0
    for index < header.import_names.len() {
        if header.import_paths[index] == options.cortado_module {
            let bound: string = header.import_names[index]
            for fixed: string in cortado_imports() {
                if fixed != bound { continue }
                locate.seek(block_start + header.import_offsets[index])
                out.diags.push(Diag.of(locate.span(),
                    "the <beans> block imports {bound} from {options.cortado_module}, and the generated file already does — cortado-bx writes `import \{{cortado_imports().join(", ")}\} from {options.cortado_module}` at the top of every file it makes, and two imports of one name is an error in a file you did not write. Remove this one"))
            }
        }
        index = index + 1
    }
    ...
    // The package is decided by where the generated file goes, because in
    // Beans a directory *is* a package and a file's `package` line does not
    // override its folder. Generated code lands in `generated/`, so that is
    // the package name; `--package` overrides it for a caller who has put the
    // output somewhere else.
    var package_name: string = options.package_name
    if package_name == "" {
        package_name = folder_name_of(directory_of(generated_path_for(path)))
    }
    if package_name == "" {
        out.diags.push(Diag.of(Span.at(1, 1),
            "{file} sits in no directory, so cortado-bx cannot tell what package it belongs to — pass --package"))
        return out
    }
    // A `package` line in the <beans> block that disagrees is refused rather
    // than silently replaced. Silently replacing it would leave the author
    // reading `package site` in their own file while every importer has to
    // write `generated`, with nothing to explain the difference.
    if header.package_name != "" && header.package_name != package_name {
        out.diags.push(Diag.of(Span.at(1, 1),
            "the <beans> block declares `package {header.package_name}`, but the generated file goes in {package_name}/ and a Beans package is its folder. Write `package {package_name}`, or leave the line out and let the folder decide"))
        return out
    }

    let body: List<string> = []
    body.push("// Generated from {path} by cortado-bx. Do not edit.")
    body.push("//")
    ...
    body.push("// render method under it is the markup, as Builder calls.")
    body.push("// Change {file} and regenerate:")
    body.push("//")
    body.push("//     cortado-bx build {path}")
    ...
    if !emitted.components.is_empty() {
        body.push("")
        body.push("// Every component tag in {file}, checked by beansc rather than by cortado-bx:")
        ...
        for tag: string in emitted.components {
            body.push("fn _cortado_component_{stem}_{mangle_tag(tag)}(value: {tag}) -> Component \{ return value \}")
        }
    }
    body.push("")
...
/// The last segment of a path — `site/checkout.bx` → `checkout.bx`.
pub fn base_name(path: string) -> string {
    let folder: string = directory_of(path)
    if folder == "" { return path }
    return path.slice(folder.len() + 1, path.len())
}
