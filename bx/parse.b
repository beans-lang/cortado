...
pub class Parser {
    ...
    fn parse_tag() -> Option<Node> {
        ...
        if component && self.peek() == 60 {
            self.report(at, "a closed generic component tag is not supported — reflection has no zero-argument initializer for a closed generic, on either backend, so cortado cannot build {tag}<...> from markup. Wrap it in a non-generic component, as in a Rows class that extends {tag}<Order>")
            self.skip_past(62)
            return none
        }
        ...
        self.lex.bump()
        // cortado has no void elements and no raw-text elements. Every control
        // is written either self-closed or with a body, and a body is always
        // parsed as markup — there is no `<script>` here whose contents are
        // not the language.
        node.children = condense(self.parse_nodes(STOP_CLOSE_TAG, component), false)
        self.expect_close(tag, at)
        return some(node)
    }

    /// A self-closed element takes no children.
    ...
    fn classify(node: ElementNode, name: string, at: Span, has_value: bool,
        ...
        if colon >= 0 {
            ...
            if !is_xml_namespace(space) {
                self.report(at, "{name} uses an attribute prefix cortado does not have — there are two, on: for an event and bind: for a two-way binding")
                return none
            }
        }
        if name == "key" {
        ...
        // An unknown attribute is refused by name rather than passed through.
        // cortado's controls have a closed set of properties, so a misspelled
        // one is a mistake and not an extension point — and a silent no-op is
        // the single most common way an interface ends up not matching the
        // markup that describes it.
        if attribute_call(name) == "" {
            let near: string = nearest_attribute(name)
            if near == "" {
                self.report(at, "<{node.tag}> has no attribute called {name} — the ones cortado knows are {attribute_list()}")
            } else {
                self.report(at, "<{node.tag}> has no attribute called {name} — did you mean {near}?")
            }
            return none
        }
        if !has_value {
    ...
    fn classify_event(node: ElementNode, event: string, at: Span, has_value: bool,
        ...
        if node.component {
            self.report(at, "on:{event} is a control event and {node.tag} is a component — a component reports an event through a Callback parameter, written on_{event}=\{...\} with the name its author gave it")
            return none
        }
        ...
        if !is_event(event) {
            ...
            if near == "" {
                self.report(at, "on:{event} is not an event cortado raises — the table is {event_list()}")
            } else {
                self.report(at, "on:{event} is not an event cortado raises — did you mean on:{near}? The table is {event_list()}")
            }
            return none
        }
    ...
    fn parse_transition(component_children: bool) -> Option<Node> {
        ...
        if word == "html" {
            // latte's one bypass: a string that goes to the browser as markup.
            // There is nothing for it to bypass here — a widget tree is objects,
            // not text — so the form is refused by name rather than left to
            // produce something that looks like it worked.
            self.report(at, "$html writes unescaped HTML and cortado draws native controls, so there is nothing for it to write into. Build the controls you want with tags")
            return none
        }
...
fn flush_text(out: List<Node>, pieces: List<string>, at: Span, has_text: bool) {
    ...
    out.push(TextNode.of(joined, at))
}

/// Whether an element's text keeps its whitespace exactly as written.
///
/// Nothing in cortado does. A control's text is a string the platform
/// measures and draws — there is no `<pre>` here whose whitespace is its
/// meaning — so every text run is condensed the same way and an author never
/// has to think about which tag they are inside.
