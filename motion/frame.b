...
pub struct Frame {
    ...
    /// Reads a frame out of the raw event the host raised.
    ///
    /// The host has one record for every kind of event, so a frame's two
    /// times travel in the fields a pointer event uses for its position. That
    /// packing is an ABI detail and it stops here — a handler is given this,
    /// never a `UiEvent` with a clock reading hidden in its `position`.
    pub static fn of(event: events.UiEvent) -> Frame {
        return Frame {
            number: event.index,
