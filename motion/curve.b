...
pub enum(u8) Curve {
    ...
    /// the translation is a `match`, the way `widgets.WidgetKind` does it, and
    /// `tools/check_constants.sh` holds the numbers to the header.
    pub fn code() -> int {
        return match self {
            linear => host.CURVE_LINEAR,
            ease_in => host.CURVE_EASE_IN,
            ease_out => host.CURVE_EASE_OUT,
            ease_in_out => host.CURVE_EASE_IN_OUT,
        }
    }

    pub static fn of(code: int) -> Curve {
        if code == host.CURVE_LINEAR { return Curve.linear }
        if code == host.CURVE_EASE_IN { return Curve.ease_in }
        if code == host.CURVE_EASE_OUT { return Curve.ease_out }
